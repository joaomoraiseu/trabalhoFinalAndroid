...
public class Gallery_AdapterRecyclerView extends RecyclerView.Adapter<Gallery_AdapterRecyclerView.ItemViewHolder> {
    private ArrayList<Foto>Fotos;
    private RecyclerViewOnClickListenerHack mRecyclerViewOnClickListenerHack;
    private Context context;

    public Gallery_AdapterRecyclerView(ArrayList<Foto> itens, Context cont) {
        this.Fotos = itens;
        this.context = cont;
    }
    ...
    public void onBindViewHolder(final ItemViewHolder ItemViewHolder, int i) {
        final Foto item = Fotos.get(i);


        if (item.getLink() != null)
            Picasso.with(context).load(item.getLink()).placeholder(R.drawable.semimagem).into(ItemViewHolder.imageview);
